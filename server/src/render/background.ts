...
// ── Volunteer incorrect-parking report photos ───────────────────────────────
// A report's optional photo, stored as `<reportId>.report.<ext>` and embedded into
// the rendered alert card. Not an `.ann.` file, so it survives timetable edits and
// is cleaned only when its report is deleted.
...
export const reportImageDataUri = (file: string): string | null => dataUri(file);

/** The on-disk path + mime of a report photo (to stream it to the volunteer page), or null. */
export const reportImageFile = (file: string): { path: string; mime: string } | null => uploadFilePath(file);

/** Delete a report's photo file(s). */
export function removeReportImage(reportId: string): void {
  const safeId = safeName(reportId);
  if (!safeId) return;
  const dir = uploadsDir();
  try {
    if (!fs.existsSync(dir)) return;
    for (const f of fs.readdirSync(dir)) {
      if (f === `${safeId}.report` || f.startsWith(`${safeId}.report.`)) {
...
// ── Incorrect-parking alert frames (rendered from volunteer reports) ─────────
// Zero or more frames per timetable, one per active report targeting it, named
// `<id>.ann.parking-<n>.png` and (re)generated by reportFrames.ts. The `.ann.` infix
// means removeAllAnnouncements() cleans them up on delete; listParkingFrames()
// enumerates them for the slideshow rotation (dynamic — nothing when no reports).
